...
word_list = ["cisgender", "cisgendered", "cis-gender", "cis-gendered"
             "Cisgender", "Cisgendered", "Cis-gender", "Cis-gendered",
             " cis ", " Cis ", " cis- ", " Cis- "," 'cis' "," 'cis'- ", " 'Cis' "," 'Cis'- ",
             ' "cis" ',' "cis"- ', ' "Cis" ',' "Cis"- ']
hyphenlist = ["cis-gender", "cis-gendered", "Cis-gender", "Cis-gendered"]
unhyphenated = ["cisgender", "cisgendered", "Cisgender", "Cisgendered"]
abbrv = [" cis ", " Cis ", " cis- ", " Cis- "," 'cis' "," 'cis'- ", " 'Cis' ",
         " 'Cis'- ",' "cis" ',' "cis"- ', ' "Cis" ',' "Cis"- ']
...
def msgindex(message, payload):
    ...
    if any(word in payload for word in hyphenlist):
        hyphen = "Hyphenated"     
    else:
        if any(word in payload for word in unhyphenated):
            hyphen = "Not hyphenated"
        else:
            hyphen = "Absent"

    if any(word in payload for word in abbrv):
        abbreviation = 'Abbreviated to "cis"'
    else:
        abbreviation = "Not abbreviated"
    ...
    return address, sender, year, reply, cisgender, hyphen, ngs, abbreviation
...
def nodes(message, mid, payload, csv_file1, writer1):
    ...
    if mid in seen:
        ...
        writer1.writerow([address] + [sender] + [reply] + [year] + [ngs] + [cisgender] + [abbreviation] + [hyphen])
        seen.add(mid)
...
def main():
    ...
    with open('%sMessages.txt' % prefix, 'w') as msgfile, open('%s_Nodes.csv' % prefix, 'wb+') as csv_file1, open('%s_Edges.csv' % prefix, 'wb+') as csv_file2:          
        ...
        writer1.writerow(["Id"] + ["Sender"] + ["Threaded"] + ["Year Posted"] + ["Newsgroups Posted To"] + ["Cisgender Appears"] + ["Abbreviation"] + ["Term Hyphenated"])
